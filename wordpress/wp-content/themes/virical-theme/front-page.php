...
<!-- Featured Categories Grid (6 ô - 2 cột x 3 hàng) -->
<section class="featured-categories-home">
    <div class="container">
        <div class="section-header">
            <h3 class="section-subtitle"><?php echo esc_html( get_theme_mod( 'virical_featured_categories_subtitle', 'DANH MỤC TIÊU BIỂU' ) ); ?></h3>
            <h2 class="section-title"><?php echo esc_html( get_theme_mod( 'virical_featured_categories_title', 'GIẢI PHÁP ÁNH SÁNG THÔNG MINH CHO MỌI KHÔNG GIAN' ) ); ?></h2>
            <div class="section-divider"></div>
        </div>

        <div class="featured-categories-grid">
            <?php for ( $i = 1; $i <= 6; $i++ ) :
                $image   = get_theme_mod( "virical_featured_cat_{$i}_image" );
                $title   = get_theme_mod( "virical_featured_cat_{$i}_title", sprintf( 'Danh mục %d', $i ) );
                $desc    = get_theme_mod( "virical_featured_cat_{$i}_desc", '' );
                $term_id = (int) get_theme_mod( "virical_featured_cat_{$i}_term", 0 );

                // Nếu đã chọn danh mục sản phẩm cha, sinh link từ đó
                if ( $term_id ) {
            ...
            ?>
                <article class="featured-category-item">
                    <a href="<?php echo esc_url( $link ); ?>" class="featured-category-link">
                        <div class="featured-category-thumb">
                            <?php if ( $image ) : ?>
                                <img src="<?php echo esc_url( $image ); ?>" alt="<?php echo esc_attr( $title ); ?>">
                            <?php else : ?>
                                <div class="featured-category-placeholder"></div>
                            <?php endif; ?>
                            <div class="featured-category-overlay">
                                <div class="featured-category-meta">
                                    <h3 class="featured-category-title"><?php echo esc_html( $title ); ?></h3>
                                    <?php if ( $desc ) : ?>
                                        <p class="featured-category-desc"><?php echo esc_html( $desc ); ?></p>
                                    <?php endif; ?>
                                    <span class="featured-category-cta">Xem chi tiết</span>
                                </div>
                            </div>
                        </div>
                    </a>
...
.featured-category-item:hover .featured-category-thumb {
